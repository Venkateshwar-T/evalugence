...
import { X, Search, Settings2, Clock, Sparkles } from 'lucide-react';
...
export default function SelectModelModal({ isOpen, onClose, onSelectModel, selectedModelName }: SelectModelModalProps) {
  ...
  return createPortal(
    <>
            ...
            <div className="flex flex-col gap-3 md:gap-4 px-4 py-3 md:px-6 md:py-5 border-b border-gray-100 dark:border-gray-800/60 bg-gray-50/50 dark:bg-gray-900/20 shrink-0">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2 md:gap-3">
                  ...
                  <div>
                    <h3 className="text-[15px] md:text-[17px] font-semibold text-gray-900 dark:text-white leading-tight">Select Model</h3>
                    <p className="text-[11px] md:text-[13px] text-gray-500 dark:text-gray-400 font-medium mt-0 md:mt-0.5">Instantly switch to any of your connected models.</p>
                  </div>
                </div>
                <button 
                  onClick={handleClose}
        ...
        <div className="flex-1 overflow-y-auto px-4 md:px-8 py-4 md:py-6 bg-white dark:bg-[#0a0a0a]">
          {!hasConnections ? (
            <div className="flex flex-col items-center justify-center h-full text-center max-w-md mx-auto">
              <div className="w-12 h-12 md:w-16 md:h-16 bg-gray-50 dark:bg-blue-900/20 rounded-xl md:rounded-2xl flex items-center justify-center mb-4 md:mb-6 border border-gray-100 dark:border-gray-800/50">
                <Settings2 className="w-6 h-6 md:w-8 md:h-8 text-gray-400" />
              </div>
              <h4 className="text-lg md:text-xl font-bold text-gray-900 dark:text-white mb-2">No Providers Connected</h4>
              <p className="text-xs md:text-sm text-gray-500 dark:text-gray-400 mb-6 md:mb-8 leading-relaxed">
                You haven't connected any API keys yet. Head over to the Models page to securely add your keys and unlock access to industry-leading AI models.
              </p>
              <Link 
                href="/models"
                onClick={handleClose}
                className="px-6 py-2.5 md:py-3 bg-black dark:bg-white text-white dark:text-black font-bold rounded-xl active:scale-95 transition-all text-xs md:text-sm hover:bg-gray-800 dark:hover:bg-gray-200 w-full"
              >
                Connect API Keys
              </Link>
            </div>
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex flex-col overflow-hidden">
                            <span className={`font-bold text-xs md:text-[13px] truncate ${isSelected ? 'text-blue-600 dark:text-blue-400' : 'text-gray-900 dark:text-white'}`}>{m.name}</span>
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex flex-col overflow-hidden flex-1">
                            <span className={`font-bold text-xs md:text-[14px] truncate ${isSelected ? 'text-blue-600 dark:text-blue-400' : 'text-gray-900 dark:text-white group-hover:text-black dark:group-hover:text-white'}`}>{m.name}</span>
